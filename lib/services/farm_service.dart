...
class FarmService with ChangeNotifier {
  ...
  Future<Farm> getFarm() async{
    ...
    final resp = await http.get('${Enviroment.apiUrl}/api/farm',
      headers:{
  ...
  Future <bool> updateFarm (Farm farm)async{
    ...
    final resp = await http.put('${Enviroment.apiUrl}/api/farm',
  ...
  Future<List<Door>> getDoors() async{
    ...
    final resp = await http.get('${Enviroment.apiUrl}/api/door/${idFarm}',
      headers:{
        'Content-Type': 'application/json',
        'x-access-token':token
      },
  ...
  Future<List<Window>> getWindows() async{
    final token = await this._storage.read(key: 'token');
    final idFarm = await this._storage.read(key: 'idFarm');
    final resp = await http.get('${Enviroment.apiUrl}/api/window/${idFarm}',
      headers: {
        'Content-Type':'application/json',
        'x-access-token' : token
      },
  ...
  Future<Belt> getBelt() async{
    final token = await this._storage.read(key: 'token');
    final idFarm = await this._storage.read(key: 'idFarm');
    final resp = await http.get('${Enviroment.apiUrl}/api/belt/${idFarm}',
      headers: {
        'Content-Type':'application/json',
        'x-access-token' : token
      },
  ...
  Future<bool> savePhoneToken() async{
    ...
    final resp = await http.post('${Enviroment.apiUrl}/api/phone',
      body: jsonEncode(data),
      headers: {
        'Content-Type':'application/json',
        'x-access-token' : token
